...
- SwiftUI 主窗口和菜单栏入口。
- 左侧菜单重构为 `状态 / Status`、`设置 / Settings`、`启动配置 / Start`、`还原配置 / Restore`、`日志 / Logs` 五栏；App 默认打开状态页。
...
- Dashboard 状态页展示 Proxy、LaunchAgent、证书、本机分流路径、准备状态，并内嵌读取 `/telemetry/summary` 的 token 用量摘要。
- 启动配置页会先探测 `node`、`npm`、`brew`、`claude`、`codex` 的真实路径；`node` 缺失会阻断安装，`npm`/`brew`/客户端命令缺失只做醒目警告。
- 启动配置页展示 `/health`、`/dashboard`、`/telemetry/summary` 与四类客户端 health URL。
...
- App 不会自动执行安装；只有用户在启动配置页完成检查和 `INSTALL` 门禁后才会运行 `launchctl`、`security add-trusted-cert` 或 `openssl`。
